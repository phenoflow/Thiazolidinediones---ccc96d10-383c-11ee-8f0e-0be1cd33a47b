...
(async () => {
  ...
  await fs.writeFile("thiazolidinediones-potential-cases.csv", "patient-id,dob,codes,last-encounter\n");
  for(let patient in patients) {
    try {
      ...
      await fs.appendFile("thiazolidinediones-potential-cases.csv", row);
    } catch(error) {
      console.log(error);
    }
  }
